...
- **Статус:** `сделано`
- **Изменённые файлы:** `vera_agent_service/app/api/v1/endpoints/chat_history.py`, `vera_agent_service/app/api/v1/endpoints/chat_session_lifecycle.py`, `vera_agent_service/app/core/settings.py`, `vera_agent_service/app/dependencies/checkpoint.py`, `vera_agent_service/app/dependencies/services.py`, `vera_agent_service/app/exceptions/chat_session.py`, `vera_agent_service/app/main.py`, `vera_agent_service/app/repositories/chat_session.py`, `vera_agent_service/app/repositories/chat_turn.py`, `vera_agent_service/app/schemas/chat_session_lifecycle.py`, `vera_agent_service/app/services/chat_history.py`, `vera_agent_service/app/services/chat_persistence.py`, `vera_agent_service/app/services/chat_session_access.py`, `vera_agent_service/app/services/chat_session_context.py`, `vera_agent_service/app/services/chat_session_lifecycle.py`, `vera_agent_service/tests/api/endpoints/test_chat_history_endpoint.py`, `vera_agent_service/tests/api/endpoints/test_chat_session_lifecycle_endpoint.py`, `vera_agent_service/tests/integration/test_chat_session_lifecycle.py`, `vera_agent_service/tests/unit/core/test_settings.py`, `vera_agent_service/tests/unit/services/test_chat_history_service.py`, `vera_agent_service/tests/unit/services/test_chat_persistence.py`, `vera_agent_service/tests/unit/services/test_chat_session_lifecycle.py`, `backend/src/api/vera.py`, `backend/src/schemas/vera.py`, `backend/src/services/vera_agent.py`, `backend/src/services/vera_session_access.py`, `backend/tests/test_vera_publisher_recovery.py`, `backend/tests/test_vera_request_correlation.py`, `backend/tests/test_vera_session_lifecycle.py`, `frontend/src/app/api/vera/chat/route.ts`, `frontend/src/app/api/vera/chat/route.test.ts`, `frontend/src/app/api/vera/session/resolve/route.ts`, `frontend/src/app/api/vera/session/resolve/route.test.ts`, `frontend/src/components/features/vera/ChatWindow.tsx`, `frontend/src/components/features/vera/__tests__/ChatWindow.test.tsx`, `frontend/src/hooks/useVeraChat.ts`, `frontend/src/hooks/__tests__/useVeraChat.test.ts`, `frontend/src/lib/api/client.ts`, `frontend/src/lib/api/vera.ts`, `frontend/src/lib/api/__tests__/vera.test.ts`, `frontend/src/lib/schemas/vera.ts`, `frontend/src/lib/schemas/__tests__/vera.test.ts`, `frontend/src/lib/utils/vera-owner-headers.ts`, `frontend/src/lib/utils/vera-session-token.ts`, `frontend/src/lib/utils/__tests__/vera-feedback-proxy.test.ts`, `frontend/src/lib/utils/__tests__/vera-response.test.ts`, `VERA_CHAT_TECHNICAL_AUDIT.md`.
- **Суть изменения:** Agent Service атомарно определяет серверную границу сессии до Rabbit publish: живой PostgreSQL-кандидат подтверждается тем же TTL и реальным Redis checkpoint, а истёкший закрывается и получает единственного successor; окно до первого checkpoint защищено живой processing-арендой. Backend сайта и Next BFF выполняют обязательный synchronous resolve, возвращают effective `session_id`, а frontend больше не ротирует ID по `createdAt`, сохраняет историю predecessor и показывает явную границу контекста. Exact bounded recovery закрывает lost-response, storage, reload и multi-tab гонки без изменения очереди или SSE; коммит Agent Service — `4e65a74`.
- **Миграции Alembic:** не требуются.
- **Тесты** (добавленные, команда запуска, результат)**:** Agent: добавлены unit/API и 11 реальных PostgreSQL+Redis lifecycle-сценариев, включая stale auth, активную старую anonymous-сессию, missing checkpoint, processing lease, concurrency и exact retry; `venv\Scripts\python.exe -m pytest -p no:cacheprovider tests\unit\core\test_settings.py tests\unit\services\test_chat_session_lifecycle.py tests\unit\services\test_chat_persistence.py tests\api\endpoints\test_chat_session_lifecycle_endpoint.py tests\api\endpoints\test_chat_history_endpoint.py tests\integration\test_chat_session_lifecycle.py -q` — `55 passed`, `11 warnings`; полный изолированный запуск `venv\Scripts\python.exe -m pytest -p no:cacheprovider -q` на `HEAD` плюс только VERA-005 diff — `301 passed`, `21 warnings`; Ruff всех изменённых Agent-файлов — `All checks passed`. Backend сайта: добавлен `backend/tests/test_vera_session_lifecycle.py`, дополнены publisher/correlation-наборы; прямой последовательный запуск всех восьми `tests\test_*.py` из `backend/` при `PYTHONPATH=.` — `51/51 OK`, `python -m compileall -q src tests` — успешно. Frontend: добавлены BFF, hook, schema/API и component-регрессии на server-only TTL, boundary/history, owner recovery, storage/reload и multi-tab; `npm test` — `235/235`, `npm run lint` — `0 errors`, `26` существующих warnings, `npm run build` — успешно (Next.js 16.1.6, TypeScript, 29 страниц).
- **Отклонения от предложенного решения и причина:** по согласованию добавлен backend сайта и расширена HTTP-квитанция chat/lifecycle-error: без синхронного канала effective `session_id` consumer уже публиковал бы сообщение в старую очередь/Redis thread, а неизменяемые queue payload и SSE не могли сообщить клиенту successor. Пятиминутный exact transport recovery и обновление подписанного owner proof не являются вторым TTL сессии и не меняют owner identity; rate limit, nginx, формат SSE, history и payload `agent.requests` не менялись.
- **Осталось / связанные карточки:** явные create/close endpoints, кнопка «Новый диалог» и политика продолжения local anonymous-сессии после входа выполняются следующей VERA-029. Late auth `Set-Cookie` от запроса, начатого до logout/смены identity, остаётся в VERA-002; восстановление anonymous proof после закрытия браузера не вводилось, поскольку меняло бы owner-семантику.
...
- **Статус:** `сделано`
- **Изменённые файлы:** `vera_agent_service/app/graph/context_budget.py` (новый), `app/graph/prompts/context.py`, `app/graph/nodes/call_kb_search.py`, `app/graph/nodes/analyze_intent.py`, `app/graph/nodes/generate_direct.py`, `app/graph/nodes/generate_with_context.py`, `app/graph/build.py`, `app/core/settings.py`, `app/main.py`, `tests/unit/graph/test_context_budget.py` (новый), `tests/unit/graph/test_context_prompts.py` (новый), `tests/unit/graph/test_call_kb_search.py`, `tests/unit/graph/test_analyze_intent.py`, `tests/unit/graph/test_generate_direct.py`, `tests/unit/graph/test_generate_with_context.py`, `tests/integration/test_graph.py`. Код и тесты зафиксированы коммитом Agent Service `f6f413a`.
- **Суть изменения:** новая настройка `GraphContextSettings` (`context_max_turns`, `context_older_turns_summary_max_chars`) задаёт явный token budget; `app/graph/context_budget.build_bounded_messages` группирует `state['messages']` в реплики и, если их больше `context_max_turns`, схлопывает более старые в одну `SystemMessage` с короткой выжимкой «Вопрос/Ответ» на реплику (без сырых tool-сообщений), сохраняя последние реплики целиком. Все три узла, вызывающие LLM (`analyze_intent`, `generate_direct`, `generate_with_context`), строят запрос из этого ограниченного представления вместо полной `state['messages']`; `state['messages']`, Redis-checkpoint и PostgreSQL-история при этом не меняются — бюджет применяется только к тому, что уходит в конкретный вызов модели. `call_kb_search` больше не кладёт в `ToolMessage` полный JSON чанков — только идентификаторы найденных чанков и короткую сводку; полный текст остаётся доступен для текущей реплики через `state['retrieved_chunks']` (используется `generate_with_context`) и отдельно персистируется как `sources`. `format_chunks_instruction` (`prompts/context.py`) оборачивает текст каждого чанка явными разделителями (`=== НАЧАЛО/КОНЕЦ ДАННЫХ ЧАНКА ===`) и предваряет их инструкцией трактовать содержимое как недоверенные данные, а не команды.
- **Миграции Alembic:** не требуются.
- **Тесты** (добавленные, команда запуска, результат)**:** `tests/unit/graph/test_context_budget.py` (реплики в пределах бюджета не меняются; старые реплики схлопываются в сводку; последний turn не теряется даже при `max_turns=1`; сырой tool-контент не попадает в сводку; сводка обрезается по `older_turns_summary_max_chars`); `tests/unit/graph/test_context_prompts.py` (чанк помечен как недоверенные данные; текст обёрнут разделителями; внедрённая в чанк команда остаётся внутри разделителей и после предупреждающей инструкции; несколько чанков — несколько блоков); новый тест в `tests/unit/graph/test_call_kb_search.py` (сырой текст чанка отсутствует в `ToolMessage`, идентификатор — присутствует); новый интеграционный тест в `tests/integration/test_graph.py` на реальном MCP-моке — 5 прошлых реплик схлопываются в одну сводку, последний вопрос всё равно доходит до модели и получает ответ (запрос содержит ровно 3 сообщения вместо 12 без бюджета). Точечный `venv\Scripts\python.exe -m pytest tests/unit/graph/test_context_budget.py tests/unit/graph/test_context_prompts.py tests/unit/graph/test_call_kb_search.py -q` — `14 passed`; `tests/integration/test_graph.py -q` — `10 passed`; полный `venv\Scripts\python.exe -m pytest -q` — `339 passed, 0 failed` (рост числа тестов относительно baseline 9 августа, `220 passed`, отражает и эту карточку, и не связанные с ней уже выполненные карточки прогонов 4–9). `ruff check` изменённых файлов — `All checks passed`.
- **Отклонения от предложенного решения и причина:** «только необходимые source excerpts» из формулировки решения реализовано как полное исключение сырого текста чанков из истории (не частичная выжимка excerpts) — карточка VERA-009 (вывод `sources[]` наружу) не в скоупе этого прогона, а текущей реплике полный текст и так доступен через `retrieved_chunks`/`sources`, поэтому усечение самих excerpts не требовалось для достижения цели карточки.
- **Осталось / связанные карточки:** нормализованный вывод `sources[]` наружу остаётся VERA-009; per-provider точный подсчёт токенов (вместо бюджета по числу реплик) в карточке не требовался и не вводился.
...
- **Статус:** `сделано`
- **Изменённые файлы:** `vera_agent_service/app/clients/mcp_client.py`, `app/graph/edges.py`, `app/graph/nodes/analyze_intent.py`, `app/graph/nodes/call_consultation_email.py`, `app/graph/nodes/call_kb_search.py`, `app/graph/policy.py`, `app/schemas/mcp_tool_results.py`, `tests/integration/test_graph.py`, `tests/integration/test_mcp_client.py`, `tests/unit/clients/test_mcp_client.py`, `tests/unit/graph/test_analyze_intent.py`, `tests/unit/graph/test_edges.py`, `tests/unit/observability/test_tracing.py`. Код и тесты зафиксированы коммитом Agent Service `0f7e4cc`.
- **Суть изменения:** `route_after_analyze_intent` (`edges.py`) отвергает `AIMessage` с более чем одним `tool_calls` вместо молчаливого выбора первого. `_parse_tool_result` (`mcp_client.py`) отвергает больше одного content-блока и проверяет разобранный `dict` по Pydantic-схеме (`KbSearchToolResult`/`ConsultationEmailToolResult`, `app/schemas/mcp_tool_results.py`) — `call_tool_with_retry`/`call_mutating_tool_once` получили обязательный параметр `result_schema`, оба узла передают свою схему. `analyze_intent` больше не выполняет `send_consultation_email` по одному намерению модели: `app/graph/policy.consultation_email_send_is_confirmed` кодово проверяет по сохранённой истории два факта из карточки (адрес в тексте текущего сообщения пользователя и запрос подтверждения в предыдущем ответе ассистента) — при их отсутствии tool_call отбрасывается, и реплика уходит как обычный текстовый ответ через уже существующую инструкцию `CONSULTATION_EMAIL_PROMPT`. Отдельно `analyze_intent` проверяет `app/graph/policy.is_probably_factual_or_legal_question` по последнему сообщению пользователя: если модель решила ответить напрямую на вопрос, похожий на фактический/правовой, код подменяет ответ синтетическим `tool_call` к `vera_rag_kb`, принудительно направляя его в поиск — честный отказ при пустом результате формирует уже существующая ветка `generate_with_context`/`NO_ANSWER_INSTRUCTION`.
- **Миграции Alembic:** не требуются.
- **Тесты** (добавленные, команда запуска, результат)**:** добавлены/расширены `tests/unit/graph/test_edges.py` (параллельные tool_calls), `tests/unit/graph/test_analyze_intent.py` (email без предшествующего подтверждения / без адреса в текущем сообщении / с подтверждением; factual-вопрос принудительно уходит в поиск; приветствие остаётся прямым ответом), `tests/unit/clients/test_mcp_client.py` и `tests/integration/test_mcp_client.py` (несколько content-блоков и результат, не прошедший схему, отвергаются), `tests/integration/test_graph.py` (двухходовая проверка подтверждения отправки на реальном MCP-моке; факт-вопрос доходит до `vera_rag_kb`, даже когда модель не вызвала тул). Точечный `venv\Scripts\python.exe -m pytest tests/unit/graph/test_edges.py tests/unit/graph/test_analyze_intent.py tests/unit/clients/test_mcp_client.py tests/unit/graph/test_call_kb_search.py tests/unit/graph/test_call_consultation_email.py -q` — `41 passed`; интеграционные `tests/integration/test_graph.py tests/integration/test_mcp_client.py` на реальном MCP-моке и Testcontainers — `15 passed`; полный `venv\Scripts\python.exe -m pytest -q` — `312 passed, 0 failed` (несколько прогонов; предыдущий baseline прогона 9 августа — `220 passed`, рост числа тестов отражает и эту карточку, и не связанные с ней уже закоммиченные VERA-005/008/011/012 из более ранних прогонов). `ruff check` изменённых файлов — `All checks passed`.
- **Отклонения от предложенного решения и причина:** пункт 3 предложенного решения («post-generation проверки grounded answer/citations») не реализован — карточка прогона 10 (`PROMPT_RUN_10.md`) прямо перечисляет четыре обязательных решения для VERA-021 и не включает эту проверку; добавление отдельного постгенерационного валидатора цитирования не запрошено и выходило бы за рамки перечисленного. Полноценный confirmation token с отдельным хранилищем не вводился — прямо исключён форматом решения из прогона.
- **Осталось / связанные карточки:** post-generation grounded-answer/citation validation, если потребуется, — отдельная карточка вне текущего прогона; вывод `sources[]` наружу остаётся VERA-009 и не затрагивался.
...
- **Статус:** `сделано` — обе независимые половины карточки закрыты.
- **Изменённые файлы:** site/backend: `backend/src/services/vera_publisher.py`, `backend/tests/test_vera_request_correlation.py`; Agent Service: `vera_agent_service/app/clients/mcp_client.py`, `vera_agent_service/app/core/settings.py`, `vera_agent_service/app/main.py`, `vera_agent_service/app/messaging/consumer.py`, `vera_agent_service/app/observability/tracing.py`, `vera_agent_service/docker-compose.yml`, `vera_agent_service/tests/integration/test_graph.py`, `vera_agent_service/tests/unit/observability/test_tracing.py`. Код Agent Service зафиксирован коммитом `05f080c`.
- **Суть изменения:** лог перед публикацией писал payload целиком, включая текст вопроса, email владельца в `user_id` и хеш токена анонимной сессии. Теперь логируются только идентификаторы и размеры: `queue`, `session_id`, `request_id`, признак авторизованности и длина сообщения — этого достаточно, чтобы связать запись с turn в PostgreSQL агента и с SSE-потоком. Сам payload очереди не урезан, ограничение касается только application logs. Коммит в репозитории сайта — `8b2c704`. В Agent Service автоматическая LangChain/LangGraph-инструментация по умолчанию скрывает inputs, outputs, messages, text, prompts и choices; полный content доступен только через явный `TRACE_CONTENT_ENABLED=true`, по умолчанию настройка выключена. Ручной root span сохраняет только IDs, размеры и метрики, а ручной KB span — длину query и число chunks без query/result. Host-порты Phoenix удалены, внутренний OTLP endpoint `phoenix:6006` сохранён.
- **Миграции Alembic:** не требуются.
- **Тесты** (добавленные, команда запуска, результат)**:** существующий `test_request_id_is_published_with_session_id` закреплял прежнее поведение и прямо требовал наличия текста вопроса в логах — переписан на проверку безопасных полей. Добавлен `test_logs_do_not_contain_question_or_owner`: вопрос, email и token hash отсутствуют в логах, но остаются в payload очереди. Запуск (в `backend/`, pytest в проекте не используется): `PYTHONPATH=. venv/Scripts/python.exe tests/test_vera_request_correlation.py` — 10/10 OK; полный набор `tests/*.py` — 19/19 OK. В Agent Service unit-тесты приведены к безопасной политике и добавлена проверка opt-in; новый integration-тест выполняет реальный compiled LangGraph через локальный MCP-сервер, подтверждает автоматический span `send_consultation_email` и отсутствие текста консультации/recipient во всех экспортированных атрибутах. `.\venv\Scripts\python.exe -m pytest tests/unit/observability/test_tracing.py tests/integration/test_graph.py -q` — `26 passed`; полный `.\venv\Scripts\python.exe -m pytest -q` — `226 passed`, `10 warnings`; `ruff` для всех изменённых Python-файлов — `All checks passed`. `docker compose config --format json` подтвердил `PhoenixPorts=null`, `PhoenixExpose="6006"` и внутренний `PHOENIX_OTLP_ENDPOINT=http://phoenix:6006/v1/traces`.
- **Отклонения от предложенного решения и причина:** нет.
- **Осталось / связанные карточки:** описание новой env-настройки относится к отложенной VERA-010; политика retention и удаление диалогов — VERA-023.
...
- **Статус:** `сделано`
- **Изменённые файлы:** `vera_agent_service/app/api/v1/endpoints/chat_session_lifecycle.py`, `vera_agent_service/app/exceptions/chat_session.py`, `vera_agent_service/app/schemas/chat_session_lifecycle.py`, `vera_agent_service/app/services/chat_session_lifecycle.py`, `vera_agent_service/tests/api/endpoints/test_chat_session_lifecycle_endpoint.py`, `vera_agent_service/tests/integration/test_chat_session_lifecycle.py`, `vera_agent_service/tests/unit/services/test_chat_session_lifecycle.py`, `backend/src/api/vera.py`, `backend/src/schemas/vera.py`, `backend/src/services/vera_agent.py`, `backend/tests/test_vera_session_commands.py`, `frontend/src/app/api/vera/session/route.ts`, `frontend/src/app/api/vera/session/route.test.ts`, `frontend/src/app/api/vera/session/[sessionId]/close/route.ts`, `frontend/src/app/api/vera/session/[sessionId]/close/route.test.ts`, `frontend/src/components/features/vera/ChatWindow.tsx`, `frontend/src/components/features/vera/__tests__/ChatWindow.test.tsx`, `frontend/src/hooks/useVeraChat.ts`, `frontend/src/hooks/__tests__/useVeraChat.test.ts`, `frontend/src/lib/api/vera.ts`, `frontend/src/lib/api/__tests__/vera.test.ts`, `frontend/src/lib/schemas/vera.ts`, `frontend/src/lib/schemas/__tests__/vera.test.ts`, `VERA_CHAT_TECHNICAL_AUDIT.md`.
- **Суть изменения:** Agent Service получил owner-checked идемпотентные create/close endpoints; закрытие сохраняет исходный `closed_at`, а exact recovery после потерянного close/create ответа атомарно переиспользует только допустимого owned successor; коммит Agent Service — `33f91c9`. Backend сайта и Next BFF проксируют команды с проверкой схемы, binding идентификатора и существующего owner proof. Frontend продолжает видимый anonymous-диалог после входа, показывает доступную кнопку «Новый диалог», текст выбранной login-policy и границу предыдущего контекста; durable bounded state machine сохраняет один и тот же кандидат при retry/reload и не переносит чужой plaintext между identity.
- **Миграции Alembic:** не требуются.
- **Тесты** (добавленные, команда запуска, результат)**:** Agent: расширены unit/API и реальные PostgreSQL+Redis lifecycle-сценарии на create/close, идемпотентность, `closed_at`, anonymous-to-auth continuation, чужого владельца, lost response и concurrent exact recovery; `venv\Scripts\python.exe -m pytest tests\unit\services\test_chat_session_lifecycle.py -q` — `30 passed`; `venv\Scripts\python.exe -m pytest tests\integration\test_chat_session_lifecycle.py -q` — `21 passed`, `21 warnings`; полный `venv\Scripts\python.exe -m pytest -q` — `355 passed`, `31 warnings`; Ruff изменённых Agent-файлов — `All checks passed`. Backend сайта: добавлен `backend/tests/test_vera_session_commands.py`; `.\venv\Scripts\python.exe tests\test_vera_session_commands.py` при `PYTHONPATH=.` — `9/9 OK`; прямой последовательный запуск всех девяти `tests\test_*.py` при `PYTHONPATH=.` — `60/60 OK`; `.\venv\Scripts\python.exe -m compileall -q src` — успешно. Frontend: добавлены route/API/schema/component/hook-регрессии на явную границу, local-first login, exact retry, crash/storage, identity/privacy и истёкший owner proof; `npm test -- --run src/hooks/__tests__/useVeraChat.test.ts src/components/features/vera/__tests__/ChatWindow.test.tsx src/app/api/vera/session/route.test.ts src/app/api/vera/session/[sessionId]/close/route.test.ts src/lib/api/__tests__/vera.test.ts src/lib/schemas/__tests__/vera.test.ts` — `181/181`; `npm test` — `287/287`; `npm run lint` — `0 errors`, `26` существующих warnings; `npm run build` — успешно (Next.js 16.1.6, TypeScript, 30 страниц).
- **Отклонения от предложенного решения и причина:** добавлен минимальный backend-прокси сайта для create/close, потому что Next BFF имеет доверенный доступ к Agent Service только через `AUTH_API_URL`, а прямых Agent URL/API key у браузерного слоя нет. Необходимые HTTP-контракты расширены, но nginx, формат SSE и history, payload `agent.requests`, owner-семантика и retention не менялись.
- **Осталось / связанные карточки:** удаление диалогов и retention остаются в VERA-023; late auth `Set-Cookie` от запроса прежней identity остаётся в VERA-002. Восстановление anonymous owner proof после закрытия браузера не добавлялось, поскольку это меняло бы owner-семантику вне скоупа; в открытой вкладке bounded recovery покрыт.

**Продуктовое решение после карточки (10 августа 2026).** Кнопка «Новый диалог» скрыта из интерфейса; критерий готовности карточки в этой части сознательно не выполняется. Причина: без списка чатов кнопка создавала видимую потерю данных — предыдущий разговор оставался в PostgreSQL, но исчезал из интерфейса после reload, потому что frontend хранит завершённые сессии только в памяти React. Для консультаций по трудовым правам это читается как удаление консультации, которую пользователь может искать позднее. Переименование в «Очистить историю» отклонено отдельно: закрытие сессии выставляет только `closed_at`, данные остаются в БД, поэтому такая кнопка обещала бы удаление, которого не происходит.

Серверная часть карточки не откатывалась: `POST /sessions` и `POST /sessions/{id}/close` вместе с прокси сайта, BFF-роутами и тестами сохранены и используются серверным lifecycle и TTL-rollover из VERA-005. Изменения только во frontend: удалён UI-элемент и обработчик в `frontend/src/components/features/vera/ChatWindow.tsx`, тест кнопки заменён на регрессию «действие не появляется в интерфейсе» плюс сохранённая проверка блокировки composer при восстановлении незавершённого создания сессии. Полный `npm test` — `288 passed`, `npm run lint` — `0 errors`, `26` существующих warnings, `npm run build` — успешно.

Действие вернётся как «+ Новый чат» в каталоге чатов, где старый диалог виден рядом в списке — см. `VERA_MULTI_CHAT_IMPLEMENTATION_PLAN.md`, этапы 1–3. До этого граница диалога полностью определяется сервером (VERA-005): сессия закрывается после 24 часов неактивности и новая создаётся автоматически.
